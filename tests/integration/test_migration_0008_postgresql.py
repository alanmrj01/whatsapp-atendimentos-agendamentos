...
async def _seed_pending_appointment() -> UUID:
    """Seed only columns that exist at migration 0008.

    This migration roundtrip intentionally runs against an old schema, so it
    must not use current ORM models that include columns introduced later.
    """
        ...
        async with engine.begin() as connection:
            ...
            await connection.execute(
                text(
                    ...
                    " pricing_type, active) "
                    "VALUES (:id, :business_id, :name, 60, 100.00, 'fixed', true)"
                ),
                {
                    "id": service_id,
                    "business_id": business_id,
            ...
            await connection.execute(
                text(
                    ...
                    "(business_id, employee_id, service_id) "
                    "VALUES (:business_id, :employee_id, :service_id)"
                ),
                {
                    "business_id": business_id,
            ...
            await connection.execute(
                text(
                    ...
                    " estimated_duration_minutes, travel_before_minutes, "
                    " travel_after_minutes, estimated_price, pricing_type, "
                    " estimate_details, quantity, access_condition) "
                    "VALUES "
                    "(:id, :business_id, :customer_id, :service_id, :employee_id, "
                    " :starts_at, :ends_at, 'pending', :notes, "
                    " 60, 0, 0, 100.00, 'fixed', '{}'::jsonb, 1, 'normal')"
                ),
                {
                    "id": appointment_id,
                    "business_id": business_id,
